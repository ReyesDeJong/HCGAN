...
changed prod[k] to prod[int(k)] in 
FeatureFunctionLib.py->SlottedA_length->slotted_autocorrelation 
...
def get_useful_FATS_features(magnitudes, times):
  ...
  useful_features = list(feature_list[np.argwhere(~np.isnan(
    feature_values.astype(
      np.float64)))])  # list(feature_list[np.where(feature_values != None)])
...
if __name__ == "__main__":
  path_to_real_data = os.path.join(PATH_TO_PROJECT, 'TSTR_data',
                                   REAL_DATA_FOLDER, REAL_DATA_NAME)
  ...
  useful_features_names = get_useful_FATS_features(train_magnitudes,
                                                   train_times)

  train_features = get_FATS(train_magnitudes, train_times,
                            useful_features_names)

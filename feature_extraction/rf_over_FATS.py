...
if __name__ == "__main__":
    ...
    path_to_features = os.path.join(PATH_TO_PROJECT, 'TSTR_data', 'datasets_original', 'REAL', '9classes_100_100', FEATURES_NAME)
    features = FATS_extractor.load_pickle(path_to_features)
    ...
    plot_confusion_matrix(
        ...
        n_class_val=0,
        title='%s Conf matrix for %i classes; Acc %.4f' % ('FATS_TRTR',
                                                           9,
                                                           np.trace(confusion_matrix) / np.sum(confusion_matrix)))
    plt.show()
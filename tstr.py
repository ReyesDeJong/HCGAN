...
## callbacks
	...
	checkpoint = ModelCheckpoint('TSTR_'+ date +'/train/'+ folder +'/weights.best.trainonsynthetic.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='max')
...
### plot acc and validation acc v/s epochs
	...
	np.save('TSTR_'+ date +'/test/'+ dataset_real +'/testonreal_score.npy', score)

	y_pred = model.predict(X_test)
	roc = roc_auc_score(y_test, y_pred)
	print('auc roc', roc)
	np.save('TSTR_'+ date +'/test/'+ dataset_real +'/testonreal_rocauc.npy', roc)
